...
- 快捷键可以修改；默认使用 `Ctrl+Alt+Z` 记忆当前活动窗口，并按程序绑定、忽略窗口标题。
- 相同匹配条件的规则以最后一条为准；较早的重复规则默认收起，点箭头展开后会在生效规则下方置灰显示为“已覆盖”。
- 支持按程序路径、进程名、窗口类和标题进行精确、包含、开头或正则匹配。
- 显示器分辨率变化后可以按工作区比例适配。
- 默认使用 Windows 窗口事件即时触发归位，并保留定时扫描兜底。
- 默认后台托盘运行；托盘菜单可暂停自动恢复、切换开机自启动。
...
关闭主窗口只会将程序收进系统托盘，自动恢复会继续工作。即使托盘入口暂时没有显示，再次运行 `WindowMemory.exe` 也会直接唤回已运行的主窗口。
